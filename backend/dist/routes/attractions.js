...
router.get('/:id', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        if (mongoose_1.default.Types.ObjectId.isValid(id)) {
            attraction = await models_1.Attraction.findById(id).lean();
        }
        if (!attraction) {
            attraction = await models_1.Attraction.findOne({ slug: id }).lean();
        }
        if (!attraction) {
        ...
        if (!req.user || req.user.role !== 'admin') {
            await models_1.Attraction.findByIdAndUpdate(attraction._id, { $inc: { views: 1 } });
        }
