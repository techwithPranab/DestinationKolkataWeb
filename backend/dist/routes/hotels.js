...
router.get('/:id', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        if (mongoose_1.default.Types.ObjectId.isValid(id)) {
            hotel = await models_1.Hotel.findById(id).lean();
        }
        if (!hotel) {
            hotel = await models_1.Hotel.findOne({ slug: id }).lean();
        }
        if (!hotel) {
        ...
        if (!req.user || req.user.role !== 'admin') {
            await models_1.Hotel.findByIdAndUpdate(hotel._id, { $inc: { views: 1 } });
        }
